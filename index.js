...
app.post('/ekle', async (req, res) => {
	...
	if (db.has("uptimeu" + kId)){
		if (db.fetch("uptimeu" + kId).length > 9) {
			res.render("hata", { islem: "Maksimum 10 adet linki uptime edebilirsin.", user: req.user })
		} else {
...
app.get("/admin", (req, res) => {
	try {
		...
		if (member.roles.cache.has(config.adminRole)) {
			...
			if (benimlinklerim.length > 0) {
				res.render("admin", { user: user, linkler: benimlinklerim, kullanici: req.user.id })
			} else {
				res.render("admin", { user: user, linkler: "Hiçbir link eklenmemiş!", kullanici: req.user.id })
			}
		} else {
